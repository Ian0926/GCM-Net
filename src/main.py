...
from models import InpaintingModel, TV_loss
...
parser.add_argument('--model_type', type=str, default='GCMNet')
parser.add_argument('--threshold', type=float, default=0.8)
parser.add_argument('--pretrained_sr', default='xx.pth', help='pretrained base model')
parser.add_argument('--pretrained', type=bool, default=False)
parser.add_argument('--save_folder', default='./checkpoint', help='Location to save checkpoint models')
parser.add_argument('--prefix', default='paris', help='Location to save checkpoint models')
...
def render(epoch, iter, mask, output, gt):
    ...
    if not os.path.exists(name_pre):
        os.makedirs(name_pre)
...
def checkpoint(epoch):
    model_out_path = opt.save_folder+'/'+'x_' + opt.model_type+"_"+opt.prefix + "_bs_{}_epoch_{}.pth".format(opt.bs, epoch)
...
#     print_network(model.discriminator)
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        ...
        for param_group in model.gen_optimizer.param_groups:
            param_group['lr'] = model.g_lr * (0.9 ** count)
...
#             param_group['lr'] = model.g_lr
        ...
        for param_group in model.dis_optimizer.param_groups:
            param_group['lr'] = model.d_lr * (0.9 ** count)
#             param_group['lr'] = model.d_lr
